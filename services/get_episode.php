...
if($requestedEpisode && $dbo){
    ...
    $query = "SELECT movie.movie_id, movie.isMovie, movie.season, movie.episode, movie.previous_ep, movie.next_ep, movie.name, movie.cover_id, cover.name, movie.portrait_id, image.name as portrait_name, movie.hour_me, movie.minute_me, movie.date_me, movie.descript FROM movie 
    LEFT JOIN image as cover ON movie.cover_id = cover.image_id 
    LEFT JOIN image ON movie.portrait_id = image.image_id 
    WHERE movie.movie_id = $requestedEpisode;";
    ...
    foreach($dbo->query($query) as $record){
        ...
        $data["portraitID"] = stripslashes($record[9]);
        $data["portraitName"] = stripslashes($record[10]);
        $data["hour_me"] = stripslashes($record[11]);
        $data["minute_me"] = stripslashes($record[12]);
        $data["date_me"] = stripslashes($record[13]);
        $data["descript"] = stripslashes($record[14]);
    }
