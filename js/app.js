...
async function renderEpisode(episodeID){
    ...
    promises.push(new Promise((resolve, reject)=>{
        fetch("./services/get_episode.php", {
            ...
            epImage.setAttribute("src", `./uploads/episodeImages/${episodeObj.portraitName}`);
            ...
            if(episodeObj.next_ep !== "0"){
                ...
                nextEpsiode.setAttribute("href", `#/episode/${episodeID}`)
            }
            // let nextEpsiode = pageRoot.querySelector(".details-sect .next-episode");
...
function searchDB(searchText){
    ...
    fetch("./services/get_search.php", {
        ...
        if(data.length > 0){
            ...
            root.innerHTML = `<p class="search-error">Error: No episode titles found containing "${searchText}" </p>`
        }
    })
}
...
function handleSearch(searchText){
    if(!searchDelay && searchText.length >= 3){
        // No active search delay, and length is minimum searchable length
        // TODO: Start a short delay that will execute a search once its over
    }else if(searchDelay && searchText.length >= 3){
        // Pending Delay, kill the current timeout and start a new one with the new search text.
    }else{
        // This case should only execute if the searchText is not yet 3 characters, disregard searchText
    }
}
